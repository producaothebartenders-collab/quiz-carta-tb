(() => {
  ...
  const els = {
    ...
    btnSelectFamily: document.getElementById("btn-select-family"),
  ...
  const RANKING_KEY_PREFIX = "tb-quiz-criacao|";
  ...
  const AUTH_KEY = "tb-quiz-criacao-drinks";
  ...
  const FAMILY_LABELS = {
    "ESPECIAIS": "Especiais",
    "MILANESE": "Milanese",
    "SOURS - DAISYS": "Sours / Daisys",
    "TIKI": "Tiki",
    "COLLINS - HIGHBALLS": "Collins / Highballs",
    "SPRITZ - CHAMPAGNE COCKTAILS": "Spritz",
    "OUTROS": "Outros",
    "SPIRIT FOWARD - MARTINIS": "Spirit Forward",
  };

  function familyLabel(familia) {
    return FAMILY_LABELS[familia] || familia;
  }

  function fillFamilyFilter() {
    if (!els.familyFilter) return;
    const families = [];
    drinks.forEach((d) => {
      if (d.familia && !families.includes(d.familia)) families.push(d.familia);
    });
    families.sort((a, b) => familyLabel(a).localeCompare(familyLabel(b), "pt-BR"));
    families.forEach((familia) => {
      const opt = document.createElement("option");
      opt.value = familia;
      const n = drinks.filter((d) => d.familia === familia).length;
      opt.textContent = `${familyLabel(familia)} (${n})`;
  ...
  function renderDrinkList() {
    ...
    sorted.forEach((drink) => {
      ...
      if (drink.familia) {
        ...
        carta.textContent = familyLabel(drink.familia);
  ...
  if (els.btnSelectFamily) {
    els.btnSelectFamily.addEventListener("click", () => {
      const familia = els.familyFilter ? els.familyFilter.value : "";
      if (!familia) {
        drinks.forEach((d) => selectedNames.add(d.name));
      } else {
        selectedNames.clear();
        drinks.filter((d) => d.familia === familia).forEach((d) => selectedNames.add(d.name));
      }
      renderDrinkList();
    });
  }
